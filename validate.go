	...
	for i := 0; i < numWorkers; i++ {
		go func() {
			...
			for solution := range unprocessed {
				err := exec.Command("ghc", "-O3", solution.Path).Run()
				if err != nil {
					log.Fatalf("failed to compile solution %d: %v", solution.Problem, err)
				}

				answer, err := exec.Command(fmt.Sprintf("./%d", solution.Problem)).CombinedOutput()
				if err != nil {
